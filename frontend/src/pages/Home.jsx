...
export default function HomePage() {
  ...
  const maskName = (fullName) => {
    ...
    if (parts.length === 1) {
      return parts[0][0] + "*".repeat(Math.max(parts[0].length - 1, 1));
    }

    // Find middle initial index (word ending with ".")
    const midIdx = parts.findIndex((p) => p.endsWith("."));

    const lastNameParts = midIdx !== -1 ? parts.slice(midIdx + 1) : [parts[parts.length - 1]];
    const firstNameParts = parts.slice(0, midIdx !== -1 ? midIdx : parts.length - 1);
    const middleInitial = midIdx !== -1 ? parts[midIdx] : null;

    // First name: 1 letter + asterisks
    const maskedFirst = firstNameParts[0][0] + "*".repeat(Math.max(firstNameParts[0].length - 1, 1));

    // Second/other first names: all asterisks
    const maskedOtherFirsts = firstNameParts.slice(1).map((n) => "*".repeat(Math.max(n.length, 1)));

    // Last name parts: 1 letter + asterisks each
    const maskedLast = lastNameParts.map((n) => n[0] + "*".repeat(Math.max(n.length - 1, 1)));

    return [maskedFirst, ...maskedOtherFirsts, middleInitial, ...maskedLast]
      .filter(Boolean)
      .join(" ");
  };
